...
/**
 * A minimized-personalized restaurant view model for a minimized-personalized
 * restaurant view that displays restaurant data in a smaller format suited
 * for the trending tab
 */
class MPRestaurantViewModel<coordinator: Coordinator>: CoordinatedGenericViewModel {
    // MARK: - Properties
    var coordinator: coordinator
    
    // MARK: - Properties
    /// The position of this view model's in some list of sibling view models, computed from the source collection in order to take
    /// the load off of the main thread when the UI is rendering
    lazy var listRank: Int = {
        return (sourceCollection
            .firstIndex(of: self.restaurantSearchResult) ?? 0) + 1
    }()
    
    let sourceCollection: [PersonalizedRestaurantSearchResult],
        imageViewSize: RestaurantImageViewModel.ImageSizes = .medium
    ...
    struct Dependencies: InjectableServices {
        let restaurantInteractor: RestaurantInteractionsManager = FPRestaurantViewModel<coordinator>.Dependencies.inject(),
            restaurantManager: RestaurantManager = FPRestaurantViewModel<coordinator>.Dependencies.inject()
    }
    ...
    let restaurantNameColor: Color = Colors.permanent_white,
        ...
        listRankBackgroundColor: Color = Colors.permanent_black.opacity(0.66),
        listRankFontColor: Color = Colors.primary_1
    ...
    let restaurantPropertiesFont: FontStyleRepository = .caption,
        ...
        restaurantAddressFont: FontStyleRepository = .body,
        listRankFont: FontStyleRepository = .subtitle,
        listRankFontWeight: UIFont.Weight = .medium
    ...
    var formattedListRank: String {
        return listRank < 10 ? "0\(listRank)" : "\(listRank)"
    }
    ...
    init(
        ...
        sourceCollection: [PersonalizedRestaurantSearchResult] = [],
