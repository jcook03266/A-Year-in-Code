...
class ResetPasswordScreenViewModel<coordinatorType: Coordinator>: CoordinatedGenericViewModel {
...
invalidInputTextColor: Color = Colors.invalid_input_red
    ...
    // MARK: - Localized Text
    let sendButtonTitle: String = LocalizedStrings
        .getLocalizedString(for: .SEND),
instructionsText: String = LocalizedStrings
        .getLocalizedString(for: .RESET_PASSWORD_SCREEN_INSTRUCTIONS),
...
unrecognizedEmailWarning: String = LocalizedStrings
        .getLocalizedString(for: .RESET_PASSWORD_SCREEN_UNKNOWN_EMAIL)
    
    var titleText: String {
        return isUserLoggedIn ?
        LocalizedStrings.getLocalizedString(for: .RESET_PASSWORD_SCREEN_TITLE_LOGGED_IN) :
        LocalizedStrings.getLocalizedString(for: .RESET_PASSWORD_SCREEN_TITLE_LOGGED_OUT)
    }
...
// MARK: - Business Logic
extension ResetPasswordScreenViewModel {
    // MARK: - Actions
    @MainActor
    var sendRequestAction: (() -> Void) {
        return { [weak self] in
            Task { @MainActor in
                ...
                guard self.dependencies.authService.canTriggerPasswordResetRequest
                else {
                    // Inform the user that they cannot send any more requests
                    self.dependencies
                        .alertManager
                        .triggerErrorAlert(alertType: .FORGOT_PASSWORD_ERROR_REQUEST_LIMIT_REACHED)
                ...
                let requestSuccessful = await self.dependencies
                    .authService
                    .triggerResetPasswordAutomation()
                ...
                guard requestSuccessful
                    ...
                    self.dependencies
                        .alertManager
                        .triggerErrorAlert(alertType: .FORGOT_PASSWORD_ERROR_REQUEST_FAILED)
