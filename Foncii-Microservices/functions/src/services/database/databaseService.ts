...
import { isNumberInRange } from "../../foncii-toolkit/utilities/commonMath";
...
class DatabaseService {
     ...
     */
    constructor(connectionURI = process.env.MONGODB_CONNECTION_URI, flushConfig: boolean = false) {
     ...
     */
    #setup() {
        if (sharedInstances[this.connection_uri].connection_uri == process.env.MONGODB_CONNECTION_URI) {
            // Dedicated instances
...
export class DatabaseServiceAdapter extends DatabaseService {
    ...
    #PaginationLimits = {
        ...
        maxResultsPerPage: 2000 // Memory constraints
    }
    ...
    constructor(databaseIdentifier: DatabaseAPI.FonciiDatabases = DatabaseAPI.FonciiDatabases.Fediverse) {
        ...
        // Parse the correct connection URI to use automatically to remove the need for extra parameters and points of failure
     ...
     * @returns -> The fetched Binary-JSON (BSON) document with its object identifier, or null if it can't be found
     */
    async findDocumentWithProperties<T>(
        collectionName: DatabaseAPI.FonciiDBCollections,
        properties: { [x: string]: any }
    ) {
        // Properties
        const collection = this.collectionRef(collectionName),
            documentFilter: any = properties;

        // Fetched document
        var document: T | null = null;
    ...
    async findDocumentsWithProperties<T extends bson.Document>({
    ...
    }): Promise<{ documents: T[], totalDocumentsLeftToPaginate: number }> {
        // Properties
        const collection = this.collectionRef(collectionName),
            documentFilter: any = properties;
        ...
        // Pagination
        let paginationPageSize = this.isPaginationPageSizeValid(resultsPerPage) ? resultsPerPage : this.#PaginationLimits.defaultResultsPerPage,
            ...
            totalDocuments = await this.countTotalDocumentsWithProperties(collectionName, properties),
            totalDocumentsLeftToPaginate = Math.max((totalDocuments - totalDocumentsPaginated), 0);
        ...
        return { documents, totalDocumentsLeftToPaginate };
    }
     ...
     */
    async countTotalDocumentsWithProperties(
        collectionName: DatabaseAPI.FonciiDBCollections,
        properties: { [x: string]: any }
    ) {
        // Properties
     ...
     */
    async doesDocumentExistWithProperties(
        collectionName: DatabaseAPI.FonciiDBCollections,
        properties: { [x: string]: any }
    ) {
        return (await this.countTotalDocumentsWithProperties(collectionName, properties)) > 0;
    }
     ...
     */
    async updateDocumentWithProperties(
        collectionName: DatabaseAPI.FonciiDBCollections,
        properties: { [x: string]: any },
     ...
     */
    async updateFieldsInDocumentWithProperties(
        collectionName: DatabaseAPI.FonciiDBCollections,
        properties: { [x: string]: any },
        ...
        try {
            ...
            logger.info(`[updateFieldsInDocumentWithProperties] Successfully updated document with properties: ${Object.entries(documentFilter)}, in the collection ${collectionName}`);
     ...
     * @returns -> True if the document was updated with the given fields successfully, false otherwise.
     */
    async updateFieldsInDocumentsWithProperties(
        collectionName: DatabaseAPI.FonciiDBCollections,
        properties: { [x: string]: any },
        updatedFields: bson.Document
    ) {
    ...
    async updateFieldsInDocumentWithID(
        ...
        return await this.updateFieldsInDocumentWithProperties(collectionName, { _id: documentID }, updatedFields);
    }
     ...
     */
    async deleteDocumentWithProperties(
        collectionName: DatabaseAPI.FonciiDBCollections,
        properties: { [x: string]: any }
    ): Promise<boolean> {
        // Properties
     ...
     */
    async deleteDocumentsWithProperties(
        collectionName: DatabaseAPI.FonciiDBCollections,
        properties: { [x: string]: any }
    ): Promise<boolean> {
        // Properties
     ...
     */
    updateFieldsInDocumentAsBulkWriteOperation(
        properties: { [x: string]: any },
        updatedFields: bson.Document
    ...
    async resolveGenericAggregationPipelineOn<T extends bson.Document>(
        ...
        try {
            ...
            logger.info(`[resolveGenericAggregationPipelineOn] Aggregation pipeline was resolved successfully on the collection ${collectionName}`);
        }
        catch (err) {
    ...
    async resolveAggregationPipelineOn<T extends bson.Document>(
        ...
        // Count total amount of documents left to paginate using custom aggregation pipeline
        const countPipeline = [...pipelineStages, { $count: "count" }], // .count to access the integer keyed to the 'count' key
            countCursor = collection.aggregate<T>(countPipeline);

        let countResult: { count: number }[] = [];

        try {
            countResult = await countCursor.toArray() as any;
            logger.info(`[resolveAggregationPipelineOn] Count pipeline was resolved successfully on the collection ${collectionName}`);
        }
        catch (err) {
            logger.error(`[resolveAggregationPipelineOn] Error encountered while resolving count pipeline on the collection ${collectionName}: ${err}`);
        }
        ...
        try {
            ...
            logger.info(`[resolveAggregationPipelineOn] Aggregation pipeline was resolved successfully on the collection ${collectionName}`);
        }
        catch (err) {
        ...
        return {
            documents,
            totalDocumentsLeftToPaginate: totalDocuments - documents.length
        };
    }
    ...
    async paginatableAggregationPipeline<T extends bson.Document>({
        ...
        // Optional sort stage
        if (sortOptions) {
            paginatablePipeline.push({ $sort: { _id: 1, ...sortOptions }}); // Sort at the end of the pipeline since all the other stages reduced the overall element count 
        }
    ...
    async autocompleteTextSearchAggregationPipeline<T extends bson.Document>({
        ...
        if (trimmedSearchQuery.length == 0) return { documents: [], totalDocumentsLeftToPaginate: 0 }
     ...
     */
    #autocompleteTextSearchClauseFactory(
        clause: {
            query: string,
            path: string
     ...
     */
    #compoundAutocompleteTextSearchClauses(
        clauses: {
            query: string,
            path: string
        }[]
